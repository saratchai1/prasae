#!/usr/bin/env python3
"""Build browser-only multi-band Sentinel-2 assets for the authoritative PDD22 dataset.

Uses the exact scene IDs already frozen in data/pdd22_satellite, the same PDD
participating-only polygon, fixed grid and cloud mask as the cleaned analytical
dataset. Output is 8-bit visualization data only; never use these PNGs for
scientific statistics or carbon accounting.
"""
...
def get_item_retry(client, scene_id):
    ...
    for attempt in range(RETRIES):
        try:
            ...
            print(f"STAC retry {attempt+1}/{RETRIES} {scene_id}: {exc}; {delay}s", flush=True)
            time.sleep(delay)
    raise RuntimeError(f"Failed STAC item {scene_id}: {last}")


def encode_band(arr, valid, path):
    scaled = np.clip((arr - REFLECTANCE_MIN) / (REFLECTANCE_MAX - REFLECTANCE_MIN), 0.0, 1.0)
    luma = np.nan_to_num(scaled * 255.0, nan=0.0).astype(np.uint8)
    alpha = np.where(valid, 255, 0).astype(np.uint8)
    Image.fromarray(np.stack([luma, alpha], axis=-1), mode="LA").save(path, "PNG", optimize=True)
...
def build_plot(client, plot):
    ...
    for obs in clean["observations"]:
        ...
        if int(obs.get("valid_pixel_count", 0)) == 0 or not scene_ids:
            dates.append({
                "month": month, "status": "no_data", "qa": obs.get("qa", "NO_DATA"),
                "coverage_pct": float(obs.get("coverage_pct", 0.0)), "files": {}, "scene_ids": []
            })
            continue
        ...
        for scene_id in scene_ids:
            ...
            clear = sat.cloud_clear_mask(scl, *[all_bands[b] for b in sat.REQUIRED_BANDS])
            masked = {}
            for band in BANDS:
        ...
        if abs(coverage - expected) > 0.35:
            raise RuntimeError(f"{code} {month} coverage mismatch spectral={coverage:.2f} clean={expected:.2f}")
        ...
        print(f"  {month} {obs.get('qa')} {coverage:.2f}% scenes={len(scene_ids)}", flush=True)
    ...
    manifest = {
        ...
        "encoding": {
            ...
            "warning": "Visualization only; analytical metrics use original float reflectance."
        },
        "rules": {
