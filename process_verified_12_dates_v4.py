#!/usr/bin/env python3
"""Mangrove metrics v4: conservative multi-index metrics + site-specific drone calibration.

Key distinctions:
- Green Cover Proxy is NDVI-threshold based and remains conservative.
- PLOT_076 (13-STC, Rayong) may use a drone-calibrated NDVI threshold only when
  calibration passes an independent spatial holdout gate.
- Other plots keep the conservative default NDVI > 0.25 until they have their own
  site-specific reference data.
- MFI remains a separate submerged-mangrove diagnostic and is never merged into cover.
- NDRE/EVI describe condition on pixels classified as Green Cover.
"""
...
PROXY_VERSION = "v4_site_calibrated_green_cover"
DEFAULT_GREEN_THRESHOLD = 0.25
CALIBRATION_PATH = base.BASE_DIR / "data" / "calibration" / "green_proxy_calibration.json"
CALIBRATED_SITE_ID = 76
...
def load_calibration():
    if not CALIBRATION_PATH.exists():
        return {
            "status": "CALIBRATION_FILE_MISSING",
            "site_id": CALIBRATED_SITE_ID,
            "selected_threshold": DEFAULT_GREEN_THRESHOLD,
        }
    try:
        payload = json.loads(CALIBRATION_PATH.read_text(encoding="utf-8"))
    except Exception as exc:
        return {
            "status": f"CALIBRATION_READ_ERROR:{type(exc).__name__}",
            "site_id": CALIBRATED_SITE_ID,
            "selected_threshold": DEFAULT_GREEN_THRESHOLD,
        }
    threshold = payload.get("selected_threshold")
    if not isinstance(threshold, (int, float)) or not 0.10 <= float(threshold) <= 0.50:
        payload["status"] = "CALIBRATION_THRESHOLD_INVALID"
        payload["selected_threshold"] = DEFAULT_GREEN_THRESHOLD
    return payload
...
def threshold_info(plot):
    plot_id = int(plot["id"])
    if plot_id == CALIBRATED_SITE_ID:
        status = str(CALIBRATION.get("status") or "UNKNOWN")
        threshold = float(CALIBRATION.get("selected_threshold", DEFAULT_GREEN_THRESHOLD))
        if status != "PROMOTED_DRONE_CALIBRATED":
            threshold = DEFAULT_GREEN_THRESHOLD
        return threshold, status, "13-STC drone-density spatial holdout"
    return DEFAULT_GREEN_THRESHOLD, "DEFAULT_UNCALIBRATED", "portfolio conservative default"


def empty_month(key, year, month, status, scenes=0, scene_meta=None, *, plot=None):
    result = base._empty_month(key, year, month, status, scenes, scene_meta)
    threshold, calibration_status, calibration_source = (
        threshold_info(plot) if plot is not None
        else (DEFAULT_GREEN_THRESHOLD, "DEFAULT_UNCALIBRATED", "portfolio conservative default")
    )
    result.update({
        "proxy_version": PROXY_VERSION,
        "green_proxy_threshold": threshold,
        "green_proxy_calibration_status": calibration_status,
        "green_proxy_calibration_source": calibration_source,
...
def build_month_v4(catalog, plot, geom, grid, inside, year, month):
    ...
    valid = (
        inside
        & np.isfinite(composite_ndvi)
        & np.isfinite(composite_ndre)
        & np.isfinite(composite_mndwi)
        & np.isfinite(composite_mfi)
    )
    ...
    rgb_uint8 = np.moveaxis(
        np.nan_to_num(
            np.clip(composite_rgb / base.RGB_REFLECTANCE_MAX * 255.0, 0, 255),
            nan=0.0,
        ).astype(np.uint8),
        0,
        -1,
    ...
    vegetation = valid & (composite_ndvi > green_threshold)
    ...
    return {
        ...
        "proxy_version": PROXY_VERSION,
        "green_proxy_threshold": round(green_threshold, 3),
        "green_proxy_calibration_status": calibration_status,
        "green_proxy_calibration_source": calibration_source,
        "scenes_used": len(scenes),
        "scene_ids": [m["id"] for m in scene_meta],
...
base.build_month = build_month_v4
_original_build_plot = base.build_plot


def build_plot_v4(plot):
    output = _original_build_plot(plot)
    threshold, calibration_status, calibration_source = threshold_info(plot)
    output["data_quality"] = "observed_only_no_interpolation_multi_index_v4"
    output["proxy_version"] = PROXY_VERSION
    output["green_proxy_threshold"] = round(threshold, 3)
    output["green_proxy_calibration_status"] = calibration_status
    output["green_proxy_calibration_source"] = calibration_source
    output["current_submerged_mangrove_signal_pct"] = (
        output["timeseries"][-1].get("submerged_mangrove_signal_pct")
    )
    ...
    if metadata_path.exists():
        ...
        metadata["green_proxy_threshold"] = round(threshold, 3)
        metadata["green_proxy_calibration_status"] = calibration_status
        metadata["green_proxy_calibration_source"] = calibration_source
        rules = metadata.setdefault("rules", {})
        rules["vegetation_proxy_version"] = PROXY_VERSION
        rules["vegetation_proxy_definition"] = (
            f"NDVI > {threshold:.3f}; conservative Green Cover proxy"
        )
        rules["calibration_scope"] = (
            "PLOT_076 uses 13-STC drone-density spatial holdout calibration when promoted; "
            "all other plots use NDVI > 0.25 conservative default."
        )
        rules["mfi_role"] = (
            f"diagnostic only: MFI > {base.SUBMERGED_MANGROVE_MFI_THRESHOLD}; "
            "not merged into vegetation coverage"
        )
        rules["submerged_mangrove_signal_definition"] = (
            f"MNDWI > {base.OPEN_WATER_MNDWI_THRESHOLD} AND "
            f"MFI > {base.SUBMERGED_MANGROVE_MFI_THRESHOLD} AND NOT vegetation_proxy"
        )
